...
```{r}
adyacencia_blo <- cbind(as.data.frame(actasbloques$BLOQUE), cmdscale(dist(actasbloques, method = "minkowski"), 1))
colnames(adyacencia_blo) <- c("BLOQUE", "X")
ggplot(adyacencia_blo, aes(X, 1, color = BLOQUE)) + geom_point() + scale_bloque + theme_blank + labs(title = "Distancias entre bloques", subtitle = "usando Minkowski")
```

...
ggplot(adyacencia_dip, aes(X, Y, color = BLOQUE)) + geom_point() + scale_bloque + theme_blank
```
...
ggplot(adyacencia_blo, aes(X, 1, color = BLOQUE)) + geom_point() + scale_bloque + theme_blank
```
...
actas_f = actas %>% left_join(actascabe, by = "acta_id")
...
for (col in 2:ncol(actasbloques_f)) {
  distancia = distancia + dist(actasbloques_f[, col])
...
plot_ly(adyacencia_f, x = ~X, y = 1, color = ~BLOQUE, frame = ~FECHA)
```
